...
/**
 * The Zlicx logo with a custom context menu for copying/navigation,
...
export function NavLogo({
  ...
  const { domain = "zlicx.com" } = useParams() as { domain: string };
        ...
        >
          <div className="grid gap-1 rounded-lg border border-gray-200 bg-white p-2 drop-shadow-sm dark:border-white/[0.15] dark:bg-black sm:min-w-[240px]">
            ...
            <ContextMenuButton
              ...
              variant="outline"
              onClick={() => window.open("https://zlicx.com/brand", "_blank")}
              icon={<BoxSelect strokeWidth={2} className="h-4 w-4" />}
            />
            {/* If it's in the app or it's a domain placeholder page (not zlicx.com homepage), show the home button */}
            {isInApp || domain != "zlicx.com" ? (
              <ContextMenuButton
                text="Home Page"
                variant="outline"
                onClick={() => window.open("https://zlicx.com", "_blank")}
              ...
              <ContextMenuButton
                text="Dashboard"
                variant="outline"
                onClick={() => window.open("https://app.zlicx.com", "_blank")}
