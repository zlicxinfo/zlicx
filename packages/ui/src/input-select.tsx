...
export function InputSelect({
        ...
        <Drawer.Portal>
          <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 mt-24 rounded-t-lg border-t border-gray-200 bg-white">
            <Command ref={commandRef} className="relative" loop>
              ...
              {openCommandList && (
                <Command.List className="zlicx-scrollbar h-[70vh] overflow-y-auto p-2">
                  {items.length === 0 &&
                    inputValue === "" &&
                    (noItemsElement ? (
  ...
  return (
    <Command ref={commandRef} className="relative" loop>
      ...
      {openCommandList && (
        <Command.List className="zlicx-scrollbar absolute z-20 mt-2 h-[calc(var(--cmdk-list-height)+17px)] max-h-[300px] w-full min-w-[160px] overflow-auto rounded-md border border-gray-200 bg-white p-2 shadow-md transition-all duration-75">
          {items.length === 0 &&
            inputValue === "" &&
            (noItemsElement ? (
              <div>{noItemsElement}</div>
