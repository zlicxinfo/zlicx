...
export const getApexDomain = (url: string) => {
  ...
  if (parts.length > 2) {
    ...
    // otherwise, it's a subdomain (e.g. zlicx.vercel.app), so we return the last 2 parts
    return parts.slice(-2).join(".");
  }
  // if it's a normal domain (e.g. zlicx.com), we return the domain
  return domain;
};
...
export const isZlicxDomain = (domain: string) => {
  return ZLICX_DOMAINS.some((d) => d.slug === domain);
};
