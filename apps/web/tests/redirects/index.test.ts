...
const poweredBy = "zlicx.com - Link management for modern marketing teams";
...
describe.runIf(env.CI)("Link Redirects", async () => {
  ...
  test("root", async () => {
    ...
    expect(response.headers.get("location")).toBe("https://zlicx.com/");
  ...
  test("with password", async () => {
    const response = await fetch(
      `${h.baseUrl}/password/check?pw=zlicx`,
      fetchOptions,
    );
