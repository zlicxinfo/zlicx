...
describe.sequential("PATCH /links/{linkId}", async () => {
  ...
  const toUpdate: Partial<Link> = {
    key: randomId(),
    url: "https://github.com/zlicx-info/zlicx",
    title: "Zlicx Software Services Pvt Ltd.",
  ...
  test("update link using linkId", async () => {
    ...
    expect(updatedLink).toStrictEqual({
      ...
      qrCode: `https://api.zlicx.com/qr?url=https://${domain}/${toUpdate.key}?qr=1`,
      tags: [],
    });
  ...
  test("update link using externalId", async () => {
    ...
    expect(linkUpdated.url).toEqual("https://github.com/zlicx-info");
  });
});
