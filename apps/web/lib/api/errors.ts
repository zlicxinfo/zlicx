...
import { capitalize } from "@zlicx/utils";
...
const ErrorSchema = z.object({
  error: z.object({
    ...
    doc_url: z.string().optional().openapi({
      ...
      example: "https://zlicx.com/docs/api-reference",
    }),
  }),
});
...
export class ZlicxApiError extends Error {
...
const docErrorUrl = "https://zlicx.com/docs/api-reference/errors";
...
export function handleApiError(error: any): ErrorResponse & { status: number } {
  ...
  // ZlicxApiError errors
  if (error instanceof ZlicxApiError) {
    return {
      error: {
        code: error.code,
