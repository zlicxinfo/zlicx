...
export default async function LinkMiddleware(
  ...
  // links on Zlicx are case insensitive by default
  ...
  if (!link) {
    ...
    if (!linkData) {
      ...
      // TODO: log 404s (https://github.com/zlicx-info/zlicx)
  ...
  if (link.projectId === LEGAL_WORKSPACE_ID) {
    return NextResponse.rewrite(new URL("/banned", req.url), {
      headers: {
        ...ZLICX_HEADERS,
        ...(!isZlicxDomain(domain) && { "X-Robots-Tag": "googlebot: noindex" }),
      },
    });
  }
  ...
  // only track the click when there is no `zlicx-no-track` header or query param
  if (
    !(
      req.headers.get("zlicx-no-track") ||
      searchParams.get("zlicx-no-track") === "1"
    )
  ) {
    ev.waitUntil(
