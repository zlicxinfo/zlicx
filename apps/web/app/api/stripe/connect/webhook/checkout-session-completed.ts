...
export async function checkoutSessionCompleted(event: Stripe.Event) {
  ...
  const zlicxCustomerId = charge.metadata?.zlicxCustomerId;
  ...
  try {
    ...
    console.log(error);
    return `Customer with zlicxCustomerId ${zlicxCustomerId} not found, skipping...`;
  }

  if (invoiceId) {
    // Skip if invoice id is already processed
    const ok = await redis.set(`zlicx_sale_events:invoiceId:${invoiceId}`, 1, {
      ex: 60 * 60 * 24 * 7,
      nx: true,
    });
  ...
  return `Checkout session completed for customer with external ID ${zlicxCustomerId} and invoice ID ${invoiceId}`;
}
