...
import { FREE_PLAN, getPlanFromPriceId, log } from "@zlicx/utils";
          ...
          users.map((user) => {
            limiter.schedule(() =>
              sendEmail({
                ...
                subject: `Thank you for upgrading to zlicx.com ${plan.name}!`,
                react: UpgradeEmail({
                  name: user.name,
                  email: user.email as string,
                  plan: plan.name,
      ...
      if (event.type === "customer.subscription.deleted") {
        ...
        await Promise.allSettled([
          ...
          workspaceUsers.map((email) =>
            sendEmail({
              email,
              from: "anandhan@zlicx.com",
              subject: "Feedback on your zlicx.com experience?",
              text: "Hey!\n\nI noticed you recently cancelled your zlicx.com subscription – we're sorry to see you go!\n\nI'd love to hear your feedback on your experience with Zlicx – what could we have done better?\n\nThanks!\n\nAnandhan S\nFounder, zlicx.com",
            }),
          ),
          workspace.domains.forEach((domain) => {
